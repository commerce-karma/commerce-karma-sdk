...
async function findClient({ id, firstName, lastName, email, phone, address, rating, createdBy, createdAt, updatedAt, archivedAt }) {
    ...
    const response = await fetch(`${commerceKarmaBackendURL}/api/customer`, {
        headers: Object.assign({ "x-Api-Key": apiKey }, removeFalsyValues({
            id,
            firstName,
            lastName,
            email,
 ...
 */
function Connect(_a) {
    var { customJSX } = _a, props = __rest(_a, ["customJSX"]);
    ...
    const onConnect = () => {
        if (isAuthed)
            return;
        if (!apiKey) {
            window.location.replace(`${commerceKarmaURL}/integration?appDomain=${window.location.hostname}&redirectTo=${window.location.href}`);
        }
    };
    const jsxProps = { onClick: () => onConnect(), disabled: isAuthed };
    return typeof customJSX === "function" ? (customJSX(jsxProps)) : (React.createElement("button", Object.assign({}, jsxProps, props), jsxProps.disabled ? "Connected" : "Connect"));
}
 ...
 */
function Client({ query, customJSX, linkProps, ratingLinkProps, nameProps, ratingProps }) {
    ...
    return typeof customJSX === "function" ? (customJSX(client)) : client ? (React.createElement("a", Object.assign({ href: `${commerceKarmaURL}/customers/${client === null || client === void 0 ? void 0 : client._id}` }, linkProps),
        React.createElement("span", Object.assign({}, nameProps), client === null || client === void 0 ? void 0 :
            client.firstName,
            " ", client === null || client === void 0 ? void 0 :
            client.lastName),
        React.createElement("span", Object.assign({}, ratingProps),
            React.createElement("b", null, client === null || client === void 0 ? void 0 : client.rating),
            " stars"))) : (React.createElement("a", Object.assign({ href: `${commerceKarmaURL}/rating` }, ratingLinkProps), "Add client to Commerce Karma"));
}

if (!window || !document) {
    throw new Error("The Commerce Karma React integration must run on the browser with access to the window and document. If you are seeing this error you could be using server components.");
}
